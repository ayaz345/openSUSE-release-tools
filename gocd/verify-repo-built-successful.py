...
if __name__ == '__main__':
    ...
    for arch in archs:
        ...
        if root.get('code') == 'finished':
            continue
        logger.error(
            f'Repository {args.project}/{args.repository}/{arch} is not yet finished'
        )
    ...
    for count in root.findall('.//statuscount'):
        ...
        if count.get('code') in ['succeeded', 'excluded', 'disabled']:
            ...
            continue
        logger.error(
            f"Repository {args.project}/{args.repository} has {count.get('code')} packages"
        )
        sys.exit(1)

    if counts['disabled'] > counts['succeeded']:
        logger.error(
            f'Repository {args.project}/{args.repository} has more disabled packages than succeeded'
        )
        sys.exit(1)
