    ...
    def _check_maintainer_review_needed(self, req, a):
        ...
        if project.startswith('openSUSE:Leap:') and hasattr(a, 'src_project'):
            ...
            if mapping is None:
                ...
                self.logger.debug(
                    f"{pkgname} comes from {origin}, submitted from {a.src_project}"
                )
                if origin.startswith('SUSE:SLE-12') and a.src_project.startswith('SUSE:SLE-12') \
                    ...
                    self.logger.info(
                        f"{pkgname} submitted from {a.src_project}, no maintainer review needed"
                    )
                    return

        if maintainers := set(maintainers_get(self.apiurl, project, pkgname)):
            ...
            if not known_maintainer:
                self.logger.debug(
                    f"author: {author}, maintainers: {','.join(maintainers)} => need review"
                )
