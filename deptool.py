...
class DepTool(cmdln.Cmdln):
        ...
        for r in repos:
            if '/' in r or r.endswith('.solv'):
                ...
                try:
                    ...
                    if not onlyenabled or parser.get(name, 'enabled') == '1':
                        repo = self.pool.add_repo(name)
        ...
        for n in packages:
            ...
            for s in sel.solvables():
                ...
                for kind in ('RECOMMENDS', 'REQUIRES', 'SUPPLEMENTS', 'ENHANCES', 'PROVIDES', 'SUGGESTS'):
                    if deps := s.lookup_deparray(
                        getattr(solv, f'SOLVABLE_{kind}'), 0
                    ):
                        print(f'  {kind}:')
                        for dep in deps:
                            print(f'    - {dep}')
    ...
    def do_rdeps(self, subcmd, opts, *args):
        ...
        for kind in kinds:
            kindid = getattr(solv, f'SOLVABLE_{kind}', 0)
            kindprinted = False
            if opts.relation:
                # FIXME: doesnt work
                for r in args:
