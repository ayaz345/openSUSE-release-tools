...
class FactorySourceChecker(ReviewBot.ReviewBot):
    ...
    def check_source_submission(self, src_project, src_package, src_rev, target_project, target_package):
        ...
        if src_srcinfo is None:
            ...
            self.logger.info(
                f"Could not get source info for {src_project}/{src_package}@{src_rev}"
            )
            return False
        projects = self._package_get_upstream_projects(target_package)
        if projects is None:
            self.logger.error(
                f"no upstream project found for {target_package}, can't check"
            )
            return False

        self.review_messages[
            'declined'
        ] = f"the package needs to be accepted in {' or '.join(projects)} first"
    ...
    def _package_get_upstream_projects(self, package):
        ...
        for prj in self.factory:
            if r := self.lookup.get(prj, package):
                projects.append(r)
...
"""
    ...
    def checkTagNotRequired(self, req, a):
        ...
        return factory_checker.check_source_submission(
            a.src_project, a.src_package, a.src_rev, a.tgt_project, a.tgt_package
        )

    def checkTagNotRequiredOrInRequest(self, req, a):
        tags = self.checkTagInRequest(req, a)
        return True if tags is True else self.checkTagNotRequired(req, a)
