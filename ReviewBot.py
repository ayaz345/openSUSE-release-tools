...
class ReviewBot(object):
    ...
    def review_mode(self, value):
        if value not in self.REVIEW_CHOICES:
            raise Exception(f"invalid review option: {value}")
    ...
    def check_requests(self):
        ...
        for req in self.requests:
            ...
            if not hasattr(req, 'staging_project'):
                staging_project = next(
                    (
                        r.by_project
                        for r in req.reviews
                        if r.state == 'new'
                        and r.by_project
                        and ":Staging:" in r.by_project
                    ),
                    None,
                )
    ...
    def request_override_check_users(self, project):
        ...
        users = []
        if group := config.get('staging-group'):
            users += group_members(self.apiurl, group)

        if self.override_group_key:
            if override_group := config.get(self.override_group_key):
    ...
    def _set_review(self, req, state):
        ...
        if doit:
            if self.dryrun:
                self.logger.info(
                    f"(dryrun) would set {req.reqid} to {state} with message {msg}"
                )
            else:
    ...
    def add_review(self, req, by_group=None, by_user=None, by_project=None, by_package=None,
        ...
        for review in req.reviews:
            if self._is_duplicate_review(review, query, allow_duplicate):
                del query['cmd']
                self.logger.debug(
                    f"skipped adding duplicate review for {'/'.join(query.values())}"
                )
                return
    ...
    def devel_project_review_needed(self, request, project, package):
        ...
        return all(review.by_user not in maintainers for review in request.reviews)
    ...
    def check_action_maintenance_release(self, req, a):
        ...
        if linkpkg is None or linkprj is None or linkprj != a.tgt_project:
            self.logger.warning(
                f"{a.src_project}/{pkgname} is not a link to {a.tgt_project}"
            )
    ...
    def check_action__default(self, req, a):
        ...
        self.review_messages['accepted'] += f': {message}'
    ...
    def get_originproject(self, project, package, rev=None):
        ...
        return originproject.text if originproject is not None else None
    ...
    def set_request_ids_search_review(self):
        ...
        if self.review_group:
            review = osc.core.xpath_join(
                review, f"@by_group='{self.review_group}' and @state='new'"
            )
        url = osc.core.makeurl(
            self.apiurl,
            ('search', 'request'),
            {
                'match': f"state/@name='review' and review[{review}]",
                'withfullhistory': 1,
            },
        )
        root = ET.parse(osc.core.http_GET(url)).getroot()
